...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/pengzai123/doubi/master/bbr.sh; then
...
Download_SSR(){
	...
	sed -i "s/API_INTERFACE = 'sspanelv2'/API_INTERFACE = 'glzjinmod'/" ${config_user_api_file}
	sed -i 's/"connect_verbose_info": 0/"connect_verbose_info": 1/' ${config_user_file}
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/pengzai123/doubi/master/service/ssrmu_centos -O /etc/init.d/ssrinstall; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrinstall
		chkconfig --add ssrinstall
		chkconfig ssrinstall on
	else
		if ! wget --no-check-certificate https://raw.githubusercontent.com/pengzai123/doubi/master/service/ssrmu_debian -O /etc/init.d/ssrinstall; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrinstall
		update-rc.d -f ssrinstall defaults
	fi
...
Uninstall_SSR(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		if [[ ! -z $(crontab -l | grep "ssrinstall.sh") ]]; then
			crontab_monitor_ssr_cron_stop
			Clear_transfer_all_cron_stop
		fi
		if [[ ${release} = "centos" ]]; then
			chkconfig --del ssrinstall
		else
			update-rc.d -f ssrinstall remove
		fi
		rm -rf ${ssr_folder} && rm -rf /etc/init.d/ssrinstall
...
Clear_transfer_all_cron_start(){
	...
	sed -i "/ssrinstall.sh/d" "$file/crontab.bak"
	echo -e "\n${Crontab_time} /bin/bash $file/ssrinstall.sh clearall" >> "$file/crontab.bak"
	crontab "$file/crontab.bak"
	rm -r "$file/crontab.bak"
	cron_config=$(crontab -l | grep "ssrinstall.sh")
...
Start_SSR(){
	...
	/etc/init.d/ssrinstall start
}
Stop_SSR(){
	SSR_installation_status
	check_pid
	[[ -z ${PID} ]] && echo -e "${Error} ShadowsocksR 未运行 !" && exit 1
	/etc/init.d/ssrinstall stop
}
Restart_SSR(){
	SSR_installation_status
	check_pid
	[[ ! -z ${PID} ]] && /etc/init.d/ssrinstall stop
	/etc/init.d/ssrinstall start
}
View_Log(){
	SSR_installation_status
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/pengzai123/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/pengzai123/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Set_crontab_monitor_ssr(){
	SSR_installation_status
	crontab_monitor_ssr_status=$(crontab -l|grep "ssrinstall.sh monitor")
...
crontab_monitor_ssr_cron_start(){
	crontab -l > "$file/crontab.bak"
	sed -i "/ssrinstall.sh monitor/d" "$file/crontab.bak"
	#curl "https://raw.githubusercontent.com/pengzai123/shadowsocksr/master/crontab.bak">"$file/crontab.bak"
	echo -e "\n* * * * * /bin/bash $file/ssrinstall.sh monitor" >> "$file/crontab.bak"
	echo -e "\n0 * * * * date>$ssr_log_file" >> "$file/crontab.bak"
	crontab "$file/crontab.bak"
	rm -r "$file/crontab.bak"
	cron_config=$(crontab -l | grep "ssrinstall.sh monitor")
	if [[ -z ${cron_config} ]]; then
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/pengzai123/doubi/master/ssrinstall.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssrinstall" ]]; then
		rm -rf /etc/init.d/ssrinstall
		Service_SSR
	fi
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/pengzai123/doubi/master/ssrinstall.sh" && chmod +x ssrinstall.sh
